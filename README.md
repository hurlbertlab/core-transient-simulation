...
4. Run R in batch mode on N cores with the run_simulation.R script: `R CMD BATCH "--args N Parms/ Results/" run_simulation.R myrun.Rout`
5. If the simulation stops before all runs are complete, restart the simulation without writing over already finished results using the 'restart_simulation.R' script (also found in the 'exec/' directory): `R CMD BATCH "--args N Parms/ Results/" restart_simulation.R myrun.Rout`
...
9. Run R in batch mode with the summarize_simulation.R script: `R CMD BATCH "--args Results/ Parms/ Summaries/" summarize_simulation.R myrun_summary.Rout`
